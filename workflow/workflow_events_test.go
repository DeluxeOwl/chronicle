...
type EventTestParams struct {
	OrderID string `json:"orderID" exhaustruct:"optional"`
}

type EventTestOutput struct {
	Result string `json:"result" exhaustruct:"optional"`
}

type ShipmentPayload struct {
	TrackingNumber string `json:"trackingNumber" exhaustruct:"optional"`
}
...
func TestWaitForEvent_BasicFlow(t *testing.T) {
	...
	wf := workflow.New(
		runner,
		"await-basic",
		func(wctx *workflow.Context, params *EventTestParams) (*EventTestOutput, error) {
			// Step 1: do some work
			...
			shipment, err := workflow.WaitForEvent[ShipmentPayload](
				wctx,
				"order.shipped:"+params.OrderID,
			)
			if err != nil {
				return nil, err
			}
...
func TestWaitForEvent_EventPublishedBeforeAwait(t *testing.T) {
	// If the event is published before the workflow reaches WaitForEvent,
	...
	wf := workflow.New(
		...
		"await-pre-publish",
		func(wctx *workflow.Context, params *EventTestParams) (*EventTestOutput, error) {
			shipment, err := workflow.WaitForEvent[ShipmentPayload](wctx, "pre-publish-event")
			if err != nil {
				return nil, err
			}
	...
	err = workflow.PublishEvent(ctx, runner, "pre-publish-event", ShipmentPayload{
	...
	// Second run: the Register call saw the published event, so GetEvent returns it
	output, err := wf.Run(ctx, instanceID)
	require.NoError(t, err)
...
func TestWaitForEvent_WithTimeout_TimesOut(t *testing.T) {
	...
	wf := workflow.New(
		...
		"await-timeout",
		func(wctx *workflow.Context, params *EventTestParams) (*EventTestOutput, error) {
			_, err := workflow.WaitForEvent[ShipmentPayload](
				wctx,
				"never-arrives",
...
func TestWaitForEvent_WithTimeout_EventArrivesBeforeTimeout(t *testing.T) {
	...
	// Publish before timeout
	clock.Advance(12 * time.Hour)
	err = workflow.PublishEvent(
		ctx,
		runner,
		"arrives-in-time",
		ShipmentPayload{TrackingNumber: "ON-TIME"},
	)
	require.NoError(t, err)
...
func TestWaitForEvent_MultipleWorkflowsWaitSameEvent(t *testing.T) {
	...
	err = workflow.PublishEvent(
		...
		runner,
		"broadcast-event",
		ShipmentPayload{TrackingNumber: "SHARED"},
	)
	require.NoError(t, err)
...
func TestWaitForEvent_FirstPublishWins(t *testing.T) {
	db := setupTestDB(t)
	runner, err := workflow.NewSqliteRunner(db)
	require.NoError(t, err)
	...
	err = workflow.PublishEvent(
		...
		runner,
		"idempotent-event",
		ShipmentPayload{TrackingNumber: "FIRST"},
	)
	require.NoError(t, err)
	...
	err = workflow.PublishEvent(
		...
		runner,
		"idempotent-event",
		ShipmentPayload{TrackingNumber: "SECOND"},
	)
	require.NoError(t, err)

	output, err := wf.Run(ctx, instanceID)
	require.NoError(t, err)
	require.Equal(t, "FIRST", output.Result, "first publish should win")
}
...
func TestWaitForEvent_WithSleep(t *testing.T) {
	...
	err = workflow.PublishEvent(
		...
		runner,
		"post-sleep-event",
		ShipmentPayload{TrackingNumber: "AFTER"},
	)
	require.NoError(t, err)
...
func TestWaitForEvent_WorkerDriven(t *testing.T) {
	...
	// Publish the event — this enqueues a wake-up task
	err = workflow.PublishEvent(
		ctx,
		runner,
		"worker-event",
		ShipmentPayload{TrackingNumber: "WORKER-OK"},
	)
	require.NoError(t, err)
...
func TestWaitForEvent_WithRetry(t *testing.T) {
	...
	err = workflow.PublishEvent(
		...
		runner,
		"retry-event",
		ShipmentPayload{TrackingNumber: "RETRY"},
	)
	require.NoError(t, err)
