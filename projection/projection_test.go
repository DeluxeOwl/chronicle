...
func TestProjectionGroup_RunAndShutdown(t *testing.T) {
	...
	accountProjection := &ProjectionMock{
		...
		HandleFunc: func(_ context.Context, rec *event.GlobalRecord) error {
			t.Logf("HandleFunc called for event with global version %d", rec.GlobalVersion()) // Helpful for debugging
			handledEvents <- rec
			return nil
		},
	}
	...
	group := projection.NewGroup(log, checkpointer, pollInterval, []projection.Projection{accountProjection})
	go group.Run(ctx)

	// Act: Append some events to the log.
	_, err := log.AppendEvents(ctx, event.LogID("account-1"), version.CheckExact(0), event.RawEvents{
	...
	for range 2 {
		...
		case <-time.After(2 * time.Second):
			t.Fatal("timed out waiting for projection to handle event")
		}
	}
	...
	require.Equal(t, 2, len(accountProjection.HandleCalls()))
}
