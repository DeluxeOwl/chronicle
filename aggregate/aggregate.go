...
type RecordedEventsFlusher interface {
	FlushRecordedEvents() []event.GenericEnvelope
}
...
type Root[TypeID ID] interface {
	...
	recordThat(Aggregate, ...event.GenericEnvelope) error
}
...
func RecordThat[TypeID ID](root Root[TypeID], events ...event.GenericEnvelope) error {
	return root.recordThat(root, events...)
}

type Base struct {
	version        version.Version
	recordedEvents []event.GenericEnvelope
}

func (br *Base) Version() version.Version { return br.version }

func (br *Base) FlushRecordedEvents() []event.GenericEnvelope {
...
//nolint:unused // False positive.
func (br *Base) recordThat(aggregate Aggregate, events ...event.GenericEnvelope) error {
